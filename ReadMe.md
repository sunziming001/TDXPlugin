...
RATE:=ABS(REF(CLOSE,0)-REF(CLOSE,1))/REF(CLOSE,1);
...
J < M3 
	AND (REF(SHORT_TREND,0)>= REF(SHORT_TREND,1) AND REF(LONG_TREND,0)>= REF(LONG_TREND,1))
	AND (RATE<=0.02 AND HIGH_RATE<=1.07)
	AND (SHORT_TREND>LONG_TREND)
	AND (CLOSE >= 0.99*LONG_TREND AND CLOSE<1.01*SHORT_TREND) 
